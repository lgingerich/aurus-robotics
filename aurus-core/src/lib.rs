...
mod hardware_tests {
    ...
    fn test_gpio_direction_control() {
        ...
        pin.set_high();
        defmt::assert!(pin.is_set_high(), "GPIO Dir Pin: Should be high for forward");
        // Test reverse direction
        pin.set_low();
        defmt::assert!(!pin.is_set_high(), "GPIO Dir Pin: Should be low for reverse");
    }
    ...
    fn test_gpio_enable_control() {
        ...
        // Test enable
        pin.set_high();
        defmt::assert!(pin.is_set_high(), "GPIO Enable Pin: Should be high when enabled");
        // Test disable
        pin.set_low();
        defmt::assert!(!pin.is_set_high(), "GPIO Enable Pin: Should be low when disabled");
    }
    ...
    fn test_pwm_channel_control() {
        ...
        // Test initial state
        defmt::assert!(!pwm_ch.is_enabled(), "PWM Channel: Should be disabled initially");
        defmt::assert_eq!(pwm_ch.current_duty_cycle(), 0, "PWM Channel: Initial duty cycle should be 0");
        // Test enable/disable
        pwm_ch.enable();
        ...
        defmt::assert_eq!(pwm_ch.current_duty_cycle(), 500, "PWM Channel: Duty cycle should be set correctly");
        // Test percentage-based control
        pwm_ch.set_duty_cycle_percent(50);
        defmt::assert_eq!(pwm_ch.current_duty_cycle(), 500, "PWM Channel: 50% should be 500/1000");
    }

    #[test]
    fn test_duty_cycle_conversion() {
        defmt::assert_eq!(duty_cycle_to_percent(500, 1000), 50, "Conversion: 500/1000 should be 50%");
        defmt::assert_eq!(duty_cycle_to_percent(0, 1000), 0, "Conversion: 0/1000 should be 0%");
        defmt::assert_eq!(duty_cycle_to_percent(1000, 1000), 100, "Conversion: 1000/1000 should be 100%");
    }
}
